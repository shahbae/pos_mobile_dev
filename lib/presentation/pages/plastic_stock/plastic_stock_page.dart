...
class PlasticStockPage extends ConsumerWidget {
  const PlasticStockPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final stockAsync = ref.watch(plasticStockListProvider);

    return Scaffold(
      backgroundColor: AppTheme.bgLight,
      appBar: AppBar(title: const Text('Stok Plastik'), centerTitle: true),
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(plasticStockListProvider),
        child: stockAsync.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (e, _) => ListView(children: [
            const SizedBox(height: 120),
            Center(child: Text('Gagal memuat stok plastik:\n$e', textAlign: TextAlign.center)),
          ]),
          data: (list) {
            if (list.isEmpty) {
              return ListView(children: const [
                SizedBox(height: 140),
                Icon(Icons.shopping_bag_outlined, size: 56, color: AppTheme.textSecondary),
                SizedBox(height: 12),
                Center(child: Text('Belum ada data stok plastik', style: TextStyle(color: AppTheme.textSecondary))),
              ]);
            }
  ...
  Widget _row(BuildContext context, WidgetRef ref, PlasticStock s, bool canAdjust) {
    return Container(
      padding: const EdgeInsets.all(16),
  ...
  Future<void> _showAdjustDialog(BuildContext context, WidgetRef ref, PlasticStock s) async {
    ...
    try {
      await ref.read(plasticRepositoryProvider).adjustPlasticStock(
            plasticId: s.plasticId,
            newQty: result,
          );
      ref.invalidate(plasticStockListProvider);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Stok plastik berhasil disesuaikan'),
