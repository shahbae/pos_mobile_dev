...
class ToppingStockPage extends ConsumerWidget {
  const ToppingStockPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final stockAsync = ref.watch(toppingStockListProvider);

    return Scaffold(
      backgroundColor: AppTheme.bgLight,
      appBar: AppBar(title: const Text('Stok Topping'), centerTitle: true),
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(toppingStockListProvider),
        child: stockAsync.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (e, _) => ListView(children: [
            const SizedBox(height: 120),
            Center(child: Text('Gagal memuat stok topping:\n$e', textAlign: TextAlign.center)),
          ]),
          data: (list) {
            if (list.isEmpty) {
              return ListView(children: const [
                SizedBox(height: 140),
                Icon(Icons.icecream_outlined, size: 56, color: AppTheme.textSecondary),
                SizedBox(height: 12),
                Center(child: Text('Belum ada data stok topping', style: TextStyle(color: AppTheme.textSecondary))),
              ]);
            }
            return ListView.separated(
              ...
              itemBuilder: (context, i) => _row(context, ref, list[i]),
            );
          },
        ),
      ),
  ...
  Widget _row(BuildContext context, WidgetRef ref, ToppingStock s) {
    return Container(
      padding: const EdgeInsets.all(16),
  ...
  Future<void> _showAdjustDialog(BuildContext context, WidgetRef ref, ToppingStock s) async {
    ...
    try {
      await ref.read(toppingRepositoryProvider).adjustToppingStock(
            toppingId: s.toppingId,
            newQty: result,
          );
      ref.invalidate(toppingStockListProvider);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Stok topping berhasil disesuaikan'),
