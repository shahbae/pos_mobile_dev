...
class ToppingStockMovementPage extends ConsumerStatefulWidget {
  const ToppingStockMovementPage({super.key});

  @override
  ConsumerState<ToppingStockMovementPage> createState() => _ToppingStockMovementPageState();
}

class _ToppingStockMovementPageState extends ConsumerState<ToppingStockMovementPage> {
  int? _selectedToppingId;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Riwayat Stok Topping'),
        backgroundColor: theme.colorScheme.surface,
        elevation: 0,
      ),
      body: Column(
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                const Text('Filter berdasarkan Topping:',
                    style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                const SizedBox(height: 8),
                toppingsAsync.when(
                  data: (toppings) => DropdownButtonFormField<int?>(
                    value: _selectedToppingId,
                    ...
                    items: [
                      const DropdownMenuItem<int?>(value: null, child: Text('Semua Topping')),
                      ...toppings.map((t) => DropdownMenuItem<int?>(value: t.id, child: Text(t.name))),
                    ],
                    onChanged: (val) => setState(() => _selectedToppingId = val),
                  ),
                  loading: () => const Center(child: CircularProgressIndicator()),
                  error: (_, __) => const Text('Gagal memuat topping'),
                ),
              ],
            ),
          ),
  ...
  Widget _item(BuildContext context, ToppingStockMovement m) {
    Color typeColor;
    IconData typeIcon;
    String typeLabel;
    switch (m.type) {
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.icecream_outlined, color: Color(0xFF6B7280), size: 22),
              ),
